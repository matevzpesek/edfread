function fixmat = fixations(edfdata, edfmeta)
...
%             if strcmp(trial_fields{f},'IMG')
%                 field_val = str2num(strvcat(regexp(strvcat(regexp(edfdata(4).IMG.msg,'\d\d.bmp','match')),'\d\d','match')));
%             else
...
%             end
        ...
        fixmat.end   = [fixmat.end, fix.end(idx)];
